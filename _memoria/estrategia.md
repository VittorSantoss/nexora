...
Construção inicial. Manoel é leigo em Claude Code e ainda está pegando o jeito de entregar sites — nenhum cliente pago ainda.
...
Dra. Elisângela Paz fechou na reunião de 19/08 — escopo travado por escrito
(LP + blog, 3 artigos publicados na entrega, R$500), documentado em
`propostas/elisangela-paz/prestacao-servico.html`. Primeira cliente paga da
Nexora. Falta: confirmar pagamento (entrada/total) e migrar
`propostas/elisangela-paz/` pra `clientes/Elisangela-Paz/` com `briefing.md`.

Prospecção ativa já virou skill: `/prospectar` (dedupe, corte de institucional, classificação em tiers por reputação × fraqueza digital, saída em `marketing/prospeccao/`). O processo de follow-up também está documentado em `marketing/prospeccao/cadencia-follow-up.md`, com o funil em `pipeline-prospeccao.xlsx` (25/08). O gargalo que resta não é mapear a rotina — é executar a cadência com constância.
